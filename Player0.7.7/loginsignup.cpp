...
void loginSignUp::RegisterButtonConnect()
{
    connect(ui->pushButton_Register,&QPushButton::clicked,[this]()
    {
        ...
        // :变量 拼接sql语句时，不直接把值放进去，而是稍后以变量赋值形式替换变量
        query.prepare("INSERT INTO UserInfo (user_name, user_passwd, salt, image) VALUES (:name, :passwd, :salt, :image);");
        // 变量绑定的形式
...
//                QPixmap mp;
            user_headimg.loadFromData(ImageData);
            ui->label_Head->setPixmap(user_headimg);
...
void loginSignUp::keyPressEvent(QKeyEvent *event)
{
    // 检查是否按下回车键
    if (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter)
    {
        ...
        {
            ui->lineEdit_Passwd->clearFocus(); // 回车时取消焦点
            handleLoginSlot();
        }
    }
}
