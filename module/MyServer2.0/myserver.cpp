...
//    generateMetadataSQL(musicDirectory);
...
void MyServer::generateMetadataSQL(const QString &musicDirectory)
{
    QDir dir(musicDirectory);
    if (!dir.exists())
    {
        qDebug() << "生成元数据SQL目录不存在：" << musicDirectory;
        return;
    }
...
//    db.setHostName("192.168.109.128");
    db.setHostName("121.40.108.31");  // 数据库服务器
    db.setUserName("root");  // 数据库用户名
    db.setPassword("1");  // 数据库密码
    db.setPort(3306);
    db.setDatabaseName("musicDB");  // 数据库名
    ...
    foreach (const QString &artistDir, dir.entryList(QDir::Dirs | QDir::NoDotAndDotDot))
    {
        ...
        foreach (const QString &albumDir, artistPath.entryList(QDir::Dirs | QDir::NoDotAndDotDot))
        {
            ...
            foreach (const QString &file, albumPath.entryList({"*.mp3", "*.flac", "*.wav"}, QDir::Files))
            {
                ...
                // 使用目录信息生成元数据并插入数据库
                query.prepare("INSERT INTO SongInfo (title, artist, album, filePath, lyricsPath, coverPath) "
                              "VALUES (:title, :artist, :album, :filePath, :lyricsPath, :coverPath)");

                query.bindValue(":title", songName.isEmpty() ? "Unknown Title" : songName);
                query.bindValue(":artist", artistDir.isEmpty() ? "Unknown Artist" : artistDir);
                query.bindValue(":album", albumDir.isEmpty() ? "Unknown Album" : albumDir);
                query.bindValue(":filePath", songPath);
                query.bindValue(":lyricsPath", lyricsFile.isEmpty() ? "N/A" : lyricsFile);
                query.bindValue(":coverPath", coverFile.isEmpty() ? "N/A" : coverFile);

                if (!query.exec())
                {
                    qDebug() << "Failed to insert song into database:" << query.lastError();
                }
            }
        }
    }
    ...
    db.close();
    qDebug() << "Music list data inserted into the database successfully!";
}
