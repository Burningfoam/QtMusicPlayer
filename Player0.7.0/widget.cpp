    ...
    bubble(new bubblePlayList(this))
{
    ...
    p->socket.connectToHost("192.168.206.112",8080);
    ...
    ui->label_enter_hotlist->setAttribute(Qt::WA_TransparentForMouseEvents);
...
void Widget::leftBarConnect()
{
    ...
    connect(ui->pushButton_page_love,&QPushButton::clicked,[this]()
    {
        ...
        {
            QMessageBox::warning(this,"打开失败","请先登录");
            return ;
        }
...
void Widget::playerBarConnect()
{
    ...
    connect(ui->toolButton_like,&QToolButton::clicked,[this]()
    {
        if(ui->toolButton_like->property("isliked") == false)
        {
            ...
            QSqlQuery query(userProcess->db); // 创建数据库查询对象
            query.prepare("INSERT INTO UserLike (user_id, music_id) VALUES (:user_id, :music_id);");
        ...
        {
            // 如果已经喜欢过这首歌，弹出提示框，或者可以进行取消喜欢的操作
            QString userId = userProcess->user_id;  // 当前用户的 ID
            QString musicId = bubble->currentMusicId();  // 获取当前正在播放的音乐 ID

            QSqlQuery query(userProcess->db);
            query.prepare("DELETE FROM UserLike WHERE user_id = :user_id AND music_id = :music_id;");
            query.bindValue(":user_id", userId);
            query.bindValue(":music_id", musicId);
...
{
    QSqlQuery query(userProcess->db);
    query.prepare("SELECT COUNT(*) FROM UserLike WHERE user_id = :user_id AND music_id = :music_id;");
    query.bindValue(":user_id", userProcess->user_id);
    query.bindValue(":music_id", bubble->currentMusicId());
...
void Widget::setRoundCornerLabel(const QPixmap &Image, QLabel *label)
{
    QPixmap image(label->width(),label->height());
    image.fill(Qt::transparent);//设置成透明背景(父窗体的背景)
...
//        IsRequestCoverReceived = false;
    ...
    if(IsRequestCoverReceived == true)
    {
        ...
        QPixmap image(ui->albumPic->width(),ui->albumPic->height());
        image.fill(Qt::transparent);//设置成透明背景(父窗体的背景)
        ...
        painter.setRenderHint(QPainter::Antialiasing);
        //设置画笔的来源图片
        painter.setBrush(albumPix.scaled(ui->albumPic->size()));
        // 绘制一个圆角矩形
        int radius = 10; // 可以根据需要调整圆角的半径
        painter.drawRoundedRect(0, 0, ui->albumPic->width(), ui->albumPic->height(), radius, radius);
